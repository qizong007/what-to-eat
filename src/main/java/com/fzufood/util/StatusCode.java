package com.fzufood.util;

public class StatusCode {

    public static final Integer SUCCESS = 0;
    public static final Integer SYSTEM_BUSY = -1;
    public static final Integer MISSING_PARAMETERS = 1001;
    public static final Integer FAIL_TO_UPDATE_DISH_STAR = 2001;
    public static final Integer FAIL_TO_UPDATE_USER_INFO = 2002;
    public static final Integer FAIL_TO_UPDATE_MARKED_WINDOW = 2003;
    public static final Integer FAIL_TO_SAVE_DISH_STAR = 3001;
    public static final Integer FAIL_TO_SAVE_FEEDBACK = 3002;
    public static final Integer INVALID_CODE = 40029;

}
